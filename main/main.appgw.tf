...
# Retrieves the self-signed certificate from Key Vault for Application Gateway
# ============================================================================

# Reference the Key Vault where SSL certificate is stored
...
# Retrieve SSL certificate from Key Vault
data "azurerm_key_vault_certificate" "sslkv_cert" {
  for_each = { for cert in var.app_gateways.ssl_certificates : cert.name => cert if(cert.key_vault_details != null)}

  # Certificate name comes from each certificate's configuration
  name         = each.value.key_vault_details.cert_name
  key_vault_id = data.azurerm_key_vault.sslkv[each.key].id

  depends_on = [azurerm_key_vault_certificate.appgw_ssl_cert]
}
...
module "application_gateway" {
  ...
  version = "0.4.3"
  name                  = var.app_gateways.name
  location              = var.location
  resource_group_name   = var.spoke_resource_group_name
  sku                   = var.app_gateways.sku
  autoscale_configuration = var.app_gateways.autoscale_configuration
  frontend_ports        = var.app_gateways.frontend_ports
  ...
  backend_address_pools = var.app_gateways.backend_address_pools 
  backend_http_settings = {
    for setting in var.app_gateways.backend_http_settings : setting.name => {
      name                  = setting.name
  ...
  gateway_ip_configuration = {
    name      = var.app_gateways.gateway_ip_configuration.name
    subnet_id = module.subnets[var.app_gateways.gateway_ip_configuration.subnet_key].resource_id
  }
  http_listeners = {
    for listener in var.app_gateways.http_listeners : listener.name => {
      name                           = listener.name
  ...
  # Maps SSL certificates from Key Vault to Application Gateway
  # ============================================================================
  ssl_certificates = {
  ...
  diagnostic_settings = {
    default = {
      name                  = "diag-${var.app_gateways.name}"
      workspace_resource_id = module.log_analytics_workspace.resource_id
      # log_categories        = each.value.log_categories
      # log_groups            = each.value.log_groups
      # metric_categories     = each.value.metric_categories
